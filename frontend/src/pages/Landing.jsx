...
export default function Landing() {
  ...
  return (
    <div className="relative min-h-screen bg-stone-50 dark:bg-slate-950 overflow-hidden">
      ...
      {/* Hero Section */}
      <section className="relative container mx-auto px-4 py-6 md:py-14 max-w-7xl min-h-0 md:min-h-[calc(100vh-5rem)] flex items-center">
        <div className="grid lg:grid-cols-2 gap-6 md:gap-12 items-center w-full">
          {/* Text Content */}
          <div className={`space-y-4 md:space-y-8 transition-all duration-1000 ease-out ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
            ...
            <h1 className="text-2xl sm:text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-gray-900 dark:text-white leading-[1.2] md:leading-[1.1]">
              Protect Every Crop Cycle{' '}
            ...
            <p className="text-sm md:text-xl text-gray-700 dark:text-gray-300 max-w-xl leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4">
              <Link
                to="/register"
                className="w-full sm:w-auto group relative px-6 py-3 md:px-8 md:py-4 rounded-xl md:rounded-2xl bg-emerald-600 text-white font-bold text-base md:text-lg shadow-lg shadow-emerald-600/20 hover:shadow-emerald-600/40 hover:-translate-y-1 transition-all flex justify-center"
              >
              ...
              <Link
                to="/login"
                className="w-full sm:w-auto px-6 py-3 md:px-8 md:py-4 rounded-xl md:rounded-2xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-semibold border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all active:scale-95 flex justify-center"
              >
                Farmer Sign In
              </Link>
            </div>
          ...
          <div className={`hidden lg:block relative transition-all duration-1000 delay-300 ease-out ${mounted ? 'opacity-100 scale-100 rotate-0' : 'opacity-0 scale-90 -rotate-2'}`}>
      ...
      <section className="container mx-auto px-4 py-8 md:py-24 max-w-7xl relative">
        <div className="text-center max-w-3xl mx-auto mb-6 md:mb-20 space-y-2 md:space-y-4">
          ...
          <p className="hidden md:block text-gray-700 dark:text-gray-300 text-base md:text-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
          {features.map((f, i) => (
            <div
              key={f.title}
              className={`group relative p-4 md:p-8 rounded-2xl md:rounded-[32px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-emerald-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-emerald-500/10 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ transitionDelay: `${i * 100}ms` }}
            >
              <div className={`w-9 h-9 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-tr ${f.gradient} flex items-center justify-center mb-3 md:mb-6 group-hover:scale-110 transition-transform duration-500`}>
                {f.icon}
              </div>
              <h3 className="text-sm md:text-xl font-bold text-gray-900 dark:text-white mb-1 md:mb-3">
                {f.title}
              </h3>
              <p className="hidden md:block text-gray-700 dark:text-gray-300 text-sm md:text-base leading-relaxed mb-6 md:h-20">
                {f.desc}
              </p>
              <div className="hidden md:flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-bold group-hover:gap-3 transition-all cursor-pointer text-sm md:text-base">
      ...
      <section className="container mx-auto px-4 py-8 md:py-24 max-w-7xl">
        <div className="relative rounded-2xl md:rounded-[40px] overflow-hidden bg-gray-900 p-5 md:p-16">
          ...
          <div className="relative grid lg:grid-cols-2 gap-6 md:gap-12 items-center">
            <div className="space-y-3 md:space-y-6">
              ...
              <p className="text-gray-400 text-xs md:text-lg leading-relaxed max-w-md">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-6">
                <div className="flex items-center gap-2 text-white">
                  <Bell className="w-4 h-4 md:w-6 md:h-6 text-amber-400 shrink-0" />
                  <span className="font-semibold text-xs md:text-sm">Early Risk Alerts</span>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <Activity className="w-4 h-4 md:w-6 md:h-6 text-emerald-500 shrink-0" />
                  <span className="font-semibold text-xs md:text-sm">Real-time Conditions</span>
                </div>
              </div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl md:rounded-3xl p-4 md:p-8 space-y-3 md:space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-indigo-400 shrink-0" />
      ...
      {/* Final CTA */}
      <section className="container mx-auto px-4 py-10 md:py-24 text-center max-w-5xl">
        <div className="space-y-4 md:space-y-8">
          <h2 className="text-2xl md:text-6xl font-bold text-gray-900 dark:text-white tracking-tight">
            Ready for a <span className="italic font-light">Healthier</span> Harvest?
          </h2>
          <p className="hidden md:block text-xl text-gray-700 dark:text-gray-300 max-w-2xl mx-auto">
            Start with clear alerts, focused advice, and full visibility across your grow rooms.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
            <Link
              to="/register"
              className="w-full sm:w-auto px-8 py-4 md:px-12 md:py-6 rounded-2xl bg-emerald-600 dark:bg-emerald-500 text-white font-bold text-base md:text-xl hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-emerald-500/20 flex items-center justify-center gap-3"
            >
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center items-center gap-6 md:gap-16 opacity-40 grayscale">
            {['Self-Hosted Ready', 'Open-Source Core', 'IoT Hardware Support'].map(b => (
              <span key={b} className="text-[10px] md:text-xs font-bold tracking-widest text-gray-500 uppercase">
                {b}
              </span>
            ))}
          </div>
