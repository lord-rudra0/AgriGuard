...
const Composer = ({ value, onChange, onSend, onUpload, onInsertAskAI, askAIActive = false, onRemoveAskAI, attachedMedia, onRemoveAttachment, uploading = false, disabled }) => {
  ...
  return (
    <form onSubmit={(e) => { triggerHaptic(); onSend(e); }} className="relative flex flex-col gap-2">
      {/* Top Row: Active Badges / Attachments */}
      {(attachedMedia || askAIActive) && (
        <div className="flex flex-wrap gap-2 items-center px-1 animate-fade-in">
          {attachedMedia && (
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-200 border border-emerald-500/30 select-none backdrop-blur-md shadow-lg shadow-emerald-500/10">
          ...
          {askAIActive && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-amber-500/20 text-amber-200 border border-amber-500/30 select-none backdrop-blur-md shadow-lg shadow-amber-500/10">
